...
/**
 * Similar to lodash's `get`, this function will resolve props of nested objects.
 * If one of the props is an array, it will also return an array and try to resolve the remaining keys from the objects nested within.
 * @param {*} object
 * @param {*} keys
 * @param {*} defaultVal
 * @returns
...
function getProp(object, keys, defaultVal) {
  ...
  if (Array.isArray(object)) {
    return object.map((o) => getProp(o, keys));
  }
...
class ImageDownloader {
  ...
  getFieldType(api, options) {
    ...
    return node ? typeof getProp(node, options.sourceField) : false;
  }

  generateSchemaType(addSchemaTypes, fieldType) {
    const schemaType =
      fieldType === "string" || !!(this.options.schemaType && this.options.schemaType === "Image")
        ? "Image"
        : "[Images]";
  ...
  async updateNodes(api, fieldType, plugin) {
    ...
    await collection.data().reduce(async (prev, node) => {
      await prev;
      const value = getProp(node, plugin.options.sourceField);
      if (value && (!Array.isArray(value) || value.length > 0)) {
        const imagePaths = await plugin.getRemoteImage(node, value, fieldType, plugin.options);
  ...
  async getRemoteImage(node, sources, fieldType, options) {
    // Set some defaults
    const {
      cache = true,
      original = false,
    ...
    return Promise.all(
      imageSources.map(async (imageSource) => {
        ...
        try {
          ...
          imageSource = normalizeUrl(imageSource, {
        ...
        var isLocal = validate(
          { imageSource: imageSource },
          { imageSource: { url: { allowLocal: downloadFromLocalNetwork } } }
        );
        ...
        try {
          ...
          await pipeline(got.stream(imageSource), fs.createWriteStream(filePath));
          ...
          console.log("");
          console.log(
            `${chalk.yellowBright(`Unable to download image for ${options.typeName} - Source URL: ${imageSource}`)}`
          );
  ...
  async run(eventName, cb, ...args) {
    ...
    for (const entry of this.api._app.plugins._listeners[eventName]) {
      ...
      const result = typeof cb === "function" ? await handler(cb(api)) : await handler(...args, api);
